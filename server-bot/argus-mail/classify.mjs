...
// ➤ available"/"if it suits you", which is how a genuine invitation is politely worded.
// ➤ WHO it hangs on is irrelevant ("if YOU are selected, we will invite you" reads as a
// ➤ real interview in EN/DE/NL), so the maybe is pinned to selected/shortlisted/successful
// ➤ a few words apart and nothing else — "a match"/"the right fit" describe the OFFER
// ➤ suiting YOU and would bin real invitations.
...
// ➤ Advice ABOUT interviews, not an invitation to one: without this every newsletter of
// ➤ interview tips reads as good news. OPENING ONLY (see classifyMessage): these words
// ➤ fill the footer of almost every HTML mail — marketing talking, not the message — and
// ➤ over a whole body they cancelled a real invitation. WHOLE WORDS ONLY: as fragments
// ➤ they hide inside innocent ones ("curso" in "recursos humanos", "guide" in
// ➤ "guidelines", which a company attaches to a real invitation). Missing an interview is
// ➤ the costliest mistake here, so anything able to cancel one must match exactly; "en
// ➤ curso" is out entirely, ordinary Spanish for "under way".
...
// ➤ The automated receipt, the commonest outcome by far. Three things it must tolerate,
// ➤ each having swallowed a real receipt: "Thanks" as well as "Thank you"; formal as well
// ➤ as informal wherever a language has both (Dutch "uw sollicitatie" — a company writing
// ➤ to a stranger is formal); and polite padding between the thanks and the "for" ("thank
// ➤ you SO MUCH for your interest"). "Reviewing your application" is here because it says
...
// ➤ THE APPLICATION NEVER ARRIVED — the mail system handing your own message back, not a
// ➤ reply from anybody: everything else says how you did, this says you never competed and
// ➤ can apply again (a real mailbox held 3). ONLY A FAILURE COUNTS: the same sender writes
// ➤ "Delivery Status Notification (Delay)" while still retrying, and a delay usually ends
...
// ➤ The part of the list above that can ONLY be a mailshot — wording no company uses to
// ➤ one person about one application. The early test uses this set, and the narrowing
// ➤ matters: "hemos encontrado" with no number after it also opens a Spanish refusal ("...
// ➤ hemos encontrado otro candidato"), which spent sixty days filed as advertising. Calls
// ➤ to action and "unsubscribe" stay out (genuine company mail carries them) but still
...
export function classifyMessage({ subject = '', snippet = '', body = '', from = '' } = {}) {
  ...
  // ➤ A MAILSHOT IS SPOTTED BEFORE ANY MEANING IS READ INTO IT: a digest carries the
  // ➤ advertising text of every vacancy it lists, and read from the bottom of the function
  // ➤ one was filed as a rejection and another as an interview against real applications
  // ➤ made elsewhere. ON THE OPENING ONLY: "unsubscribe" sits in the footer of nearly every
  // ➤ company mail.
